...
function OfferModal({ isOpen, onClose, onSave, editing, categories, products }) {
 ...
 <h2 className="text-sm font-semibold text-gray-900">{editing?'Edit Offer':'Create Special Offer'}</h2>
 ...
 {/* Offer Type */}
 <div><label className="text-xs font-medium text-label block mb-2">Offer Type</label>
 ...
 <p className="text-xs font-medium text-gray-800">{l}</p>
 </button>
 ))}
 </div>
 </div>
 {form.offerType==='category'&&(
 <div><label className="text-xs font-medium text-label block mb-1.5">Link to Category</label>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Link to Product</label>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Select Products ({form.multiProdIds.length})</label>
 ...
 <span className="text-xs text-muted">{formatINR(p.price)}</span>
 </label>
 ))}
 </div></div>
 )}
 <div className="grid grid-cols-3 gap-3">
 <div><label className="text-xs font-medium text-label block mb-1.5">Start Date</label>
 <input type="date" value={form.startDate} onChange={e=>set('startDate',e.target.value)} className={fi}/></div>
 <div><label className="text-xs font-medium text-label block mb-1.5">End Date</label>
 ...
 <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 rounded-xl text-xs font-medium text-white disabled:opacity-50 flex items-center justify-center gap-1.5" style={{background:'linear-gradient(135deg,#7c3aed,#9d5cf6)'}}>
...
export default function SpecialOffersPage() {
 ...
 <div><h1 className="text-page-title text-gray-900 flex items-center gap-2"><Gift size={18} className="text-violet-600"/>Special Offers</h1>
 <p className="text-xs text-muted mt-0.5">{offers.length} offers ... {offers.filter(isActive).length} active</p></div>
 <button onClick={()=>{setEditing(null);setModalOpen(true);}} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium text-white shadow-md transition" style={{background:'linear-gradient(135deg,#7c3aed,#9d5cf6)'}}>
 ...
 <p className="text-sm font-medium text-gray-600">No special offers yet</p>
 <button onClick={()=>{setEditing(null);setModalOpen(true);}} className="mt-4 flex items-center gap-1.5 px-4 py-2 text-white text-xs font-medium rounded-xl" style={{background:'linear-gradient(135deg,#7c3aed,#9d5cf6)'}}><Plus size={12}/>Create First Offer</button>
 </div>
 ) : (
 ...
 <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex-shrink-0 ${active?'bg-emerald-100 text-emerald-700':offer.status==='scheduled'?'bg-amber-100 text-amber-700':'bg-gray-100 text-gray-500'}`}>
 ...
 {offer.badgeText&&<span className="text-xs font-medium px-2 py-0.5 rounded-full text-white" style={{background:'#7c3aed'}}>{offer.badgeText}</span>}
 <span className="text-xs text-muted capitalize">{offer.offerType} offer</span>
 {(offer.startDate||offer.endDate)&&<span className="text-xs text-muted">{offer.startDate||'...'} ? {offer.endDate||'...'}</span>}
 </div>
 </div>
 ...
 <span className="text-xs font-medium text-muted text-center">{offer.displayOrder||0}</span>
