...
function StatusBadge({ status }) {
 ...
 return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border capitalize ${m[status]||m.draft}`}><span className={`w-1.5 h-1.5 rounded-full ${status==='published'?'bg-green-500':status==='draft'?'bg-yellow-400':'bg-gray-400'}`}/>{status}</span>;
}
function StockBadge({ stock }) {
 const s = Number(stock);
 if (s===0) return <span className="text-[10px] font-medium text-red-600 bg-red-50 px-2 py-0.5 rounded-full border border-red-200">Out of Stock</span>;
 if (s<=10) return <span className="text-[10px] font-medium text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded-full border border-yellow-200">Low: {s}</span>;
 return <span className="text-[10px] font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded-full border border-green-200">{s} units</span>;
}

function ProductCard({ prod, categoryName, onEdit, onStock, onToggle, onDelete }) {
 const disc = prod.mrp > prod.price ? Math.round(((prod.mrp - prod.price) / prod.mrp) * 100) : 0;
 const img = prod.images?.[0];
 return (
 <article className="bg-white border border-gray-100 rounded-2xl p-3 shadow-sm">
 <div className="flex gap-3">
 <div className="w-24 h-24 rounded-xl overflow-hidden border border-gray-100 bg-gray-50 flex items-center justify-center flex-shrink-0">
 {img ? <img src={img} alt={prod.name} className="w-full h-full object-cover" onError={e=>{e.target.src='/logo.png'}}/> : <img src="/logo.png" alt={prod.name} className="w-full h-full object-contain p-2"/>}
 </div>
 <div className="min-w-0 flex-1">
 <h2 className="text-sm font-semibold text-gray-900 leading-snug break-words">{prod.name}</h2>
 <p className="text-xs text-muted font-mono mt-1 break-all">{prod.sku || 'No SKU'}</p>
 <p className="text-xs text-gray-500 mt-1 break-words">{prod.brand || 'No brand'}</p>
 ...
 <div className="rounded-xl bg-gray-50 p-2 min-w-0">
 <p className="text-xs text-muted font-medium uppercase">Category</p>
 <p className="text-gray-800 font-medium mt-0.5 break-words">{categoryName}</p>
 </div>
 <div className="rounded-xl bg-gray-50 p-2">
 <p className="text-xs text-muted font-medium uppercase">Unit</p>
 <p className="text-gray-800 font-medium mt-0.5">{prod.unit || '-'}</p>
 </div>
 <div className="rounded-xl bg-gray-50 p-2">
 <p className="text-xs text-muted font-medium uppercase">Price</p>
 <p className="text-gray-900 font-semibold mt-0.5">{prod.price ? formatINR(prod.price) : '-'}</p>
 </div>
 <div className="rounded-xl bg-gray-50 p-2">
 <p className="text-xs text-muted font-medium uppercase">MRP / Discount</p>
 <p className="text-gray-800 font-medium mt-0.5">{prod.mrp ? formatINR(prod.mrp) : '-'} {disc > 0 ? <span className="text-green-700">({disc}% off)</span> : null}</p>
 </div>
 </div>
 <div className="grid grid-cols-4 gap-2 mt-3">
 <button onClick={onEdit} className="min-h-10 rounded-xl bg-blue-50 text-blue-700 flex items-center justify-center" aria-label="Edit product"><Edit2 size={16}/></button>
 <button onClick={onStock} className="min-h-10 rounded-xl bg-green-50 text-green-700 flex items-center justify-center" aria-label="Update stock"><RefreshCw size={16}/></button>
 <button onClick={onToggle} className="min-h-10 rounded-xl bg-yellow-50 text-yellow-700 flex items-center justify-center" aria-label="Toggle status">{prod.status==='published'?<ToggleRight size={17}/>:<ToggleLeft size={17}/>}</button>
 <button onClick={onDelete} className="min-h-10 rounded-xl bg-red-50 text-red-700 flex items-center justify-center" aria-label="Delete product"><Trash2 size={16}/></button>
 </div>
 </article>
 );
}

/* â”€â”€ Quick Stock Modal â”€â”€ */
function StockModal({ product, onClose, onSave }) {
 ...
 <h3 className="text-sm font-semibold text-gray-900 mb-1">Update Stock</h3>
 ...
 className="w-full border border-gray-200 rounded-xl px-4 py-3 text-lg font-semibold text-center focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary mb-4"/>
 <div className="flex gap-3">
 ...
 className="flex-1 py-2.5 rounded-xl text-xs font-medium text-white bg-primary hover:bg-primary-hover disabled:opacity-50 flex items-center justify-center gap-1.5">
...
function ProductModal({ isOpen, onClose, onSave, editing, categories, pincodes }) {
 ...
 <div className="bg-white rounded-2xl sm:rounded-3xl w-full flex flex-col animate-pop"
 style={{maxWidth:'660px',maxHeight:'calc(100vh - 24px)',boxShadow:'0 32px 80px rgba(0,0,0,0.28)'}}
 onClick={e=>e.stopPropagation()}>

 {/* Header */}
 <div className="flex items-center justify-between px-4 sm:px-7 pt-5 sm:pt-6 pb-4 border-b border-gray-100 flex-shrink-0">
 <div>
 <h2 className="text-sm font-semibold text-gray-900">
 {editing ? 'Edit Product' : 'Add New Product'}
 </h2>
 <p className="text-xs text-muted mt-0.5">{editing ? 'Update product details' : 'Fill details to add a product'}</p>
 </div>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Name *</label>
 <input value={form.name} onChange={e=>set('name',e.target.value)} placeholder="Product name" className={fi}/></div>
 <div><label className="text-xs font-medium text-label block mb-1.5">Brand</label>
 <input value={form.brand} onChange={e=>set('brand',e.target.value)} placeholder="Brand" className={fi}/></div>
 </div>

 <div><label className="text-xs font-medium text-label block mb-1.5">Description</label>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Discount %</label>
 <input value={(form.discountPercent||0)+'%'} readOnly
 className={fi+' bg-gray-50 text-center font-medium text-primary cursor-default'}/></div>
 </div>

 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <div><label className="text-xs font-medium text-label block mb-1.5">SKU</label>
 <input value={form.sku} onChange={e=>set('sku',e.target.value)} className={fi}/></div>
 <div><label className="text-xs font-medium text-label block mb-1.5">Barcode</label>
 <input value={form.barcode||''} onChange={e=>set('barcode',e.target.value)} className={fi}/></div>
 <div><label className="text-xs font-medium text-label block mb-1.5">Expiry Date</label>
 ...
 <p className="text-xs font-medium text-label mb-2.5">Attributes</p>
 ...
 className={`px-2 py-0.5 rounded text-[11px] font-medium border transition ${active?'bg-primary text-white border-primary':'bg-gray-50 text-gray-500 border-gray-200'}`}>{code}</button>;
 })}
 </div>
 </div>
 )}
 ...
 {/* Images â€” using ImageUploader (supports upload + URL) */}
 <div className="space-y-3">
 <ImageUploader
 ...
 <div>
 <label className="text-xs font-medium text-label block mb-2">
 ...
 className="opacity-0 group-hover:opacity-100 transition text-[10px] font-medium text-white bg-red-500 px-1.5 py-1 rounded-lg">âœ•</button>
 </div>
 {i===0&&<span className="absolute top-1 left-1 bg-primary text-white text-[10px] font-medium px-1 py-0.5 rounded">Main</span>}
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Footer */}
 <div className="flex flex-col sm:flex-row gap-3 px-4 sm:px-7 pb-5 sm:pb-6 pt-4 border-t border-gray-100 flex-shrink-0">
 <button onClick={onClose} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50">Cancel</button>
...
export default function ProductsPage() {
 ...
 const getCatName = id => categories.find(c => c.id === id)?.name || 'â€”';
 ...
 <select value={filterCat} onChange={e=>{setFilterCat(e.target.value);setPage(1);}} className="w-full sm:w-auto px-3 py-2.5 sm:py-2 text-xs border border-gray-200 rounded-xl bg-white font-medium text-gray-700">
 ...
 {/* Products */}
 <div className="flex-1 overflow-auto px-4 sm:px-6 py-4 pb-24 lg:pb-4 min-w-0">
 ...
 key={prod.id}
 prod={prod}
 categoryName={getCatName(prod.category)}
 onEdit={()=>navigate(`/products/edit/${prod.id}`)}
 onStock={()=>setStockProd(prod)}
 onToggle={()=>handleToggle(prod)}
 onDelete={()=>setConfirmDel(prod.id)}
 />
 ))}
 </div>
 ...
 <th className="px-3 py-3 text-left text-xs font-medium text-muted uppercase w-12">Image</th>
 {[['name','Product'],['brand','Brand'],['category','Category']].map(([k,l])=>(
 <th key={k} onClick={()=>toggleSort(k)} className="px-3 py-3 text-left text-xs font-medium text-muted uppercase cursor-pointer hover:text-gray-700 select-none">
 <span className="flex items-center gap-1">{l}<SI k={k}/></span>
 </th>
 ))}
 <th className="px-3 py-3 text-left text-xs font-medium text-muted uppercase">Unit</th>
 <th onClick={()=>toggleSort('mrp')} className="px-3 py-3 text-right text-xs font-medium text-muted uppercase cursor-pointer select-none"><span className="flex items-center justify-end gap-1">MRP<SI k="mrp"/></span></th>
 <th onClick={()=>toggleSort('price')} className="px-3 py-3 text-right text-xs font-medium text-muted uppercase cursor-pointer select-none"><span className="flex items-center justify-end gap-1">Price<SI k="price"/></span></th>
 <th className="px-3 py-3 text-center text-xs font-medium text-muted uppercase">Disc%</th>
 <th onClick={()=>toggleSort('stock')} className="px-3 py-3 text-center text-xs font-medium text-muted uppercase cursor-pointer select-none"><span className="flex items-center justify-center gap-1">Stock<SI k="stock"/></span></th>
 <th className="px-3 py-3 text-center text-xs font-medium text-muted uppercase">Status</th>
 <th className="px-3 py-3 text-center text-xs font-medium text-muted uppercase">Actions</th>
 </tr>
 </thead>
 ...
 <button onClick={()=>navigate(`/products/edit/${prod.id}`)} title="Edit" className="w-7 h-7 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 flex items-center justify-center transition"><Edit2 size={12}/></button>
 ...
 {/* Pagination */}
 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-4 text-xs text-gray-500">
 <span>{filtered.length} results Â· Page {page} of {total}</span>
 ...
 return <button key={p} onClick={()=>setPage(p)} className={`w-8 h-8 rounded-lg border text-xs font-medium ${p===page?'bg-primary text-white border-primary':'border-gray-200 hover:bg-gray-50 text-gray-600'}`}>{p}</button>;
 })}
