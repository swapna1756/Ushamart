...
function BannerModal({ isOpen, onClose, onSave, editing }) {
 ...
 <h2 className="text-sm font-semibold text-gray-900">{editing?'Edit Banner':'New Banner'}</h2>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Badge</label>
 <input value={form.badgeText} onChange={e=>set('badgeText',e.target.value)} placeholder="SPECIAL" className={fi}/></div>
 <div><label className="text-xs font-medium text-label block mb-1.5">Button</label>
 <input value={form.buttonText} onChange={e=>set('buttonText',e.target.value)} className={fi}/></div>
 </div>
 <div className="grid grid-cols-2 gap-3">
 <div><label className="text-xs font-medium text-label block mb-1.5">BG Color</label>
 ...
 <div><label className="text-xs font-medium text-label block mb-1.5">Order</label>
...
export default function BannersPage() {
 ...
 <div><h1 className="text-page-title text-gray-900 flex items-center gap-2"><Megaphone size={18} className="text-blue-500"/>Banners</h1>
 <p className="text-xs text-muted mt-0.5">{banners.length} banners � {banners.filter(b=>b.active).length} active</p></div>
 ...
 <p className="text-sm font-medium text-gray-600">No banners yet</p>
 <button onClick={()=>{setEditing(null);setModalOpen(true);}} className="mt-4 flex items-center gap-1.5 px-4 py-2 bg-primary text-white text-xs font-medium rounded-xl"><Plus size={12}/>Create Banner</button>
 </div>
 ) : (
 ...
 <div>
 {b.badgeText&&<span className="text-[10px] font-medium bg-white/20 text-white px-2 py-0.5 rounded-full backdrop-blur-sm">{b.badgeText}</span>}
 <p className="text-white font-semibold text-sm mt-1 drop-shadow">{b.title}</p>
 {b.subtitle&&<p className="text-white/80 text-[10px] mt-0.5">{b.subtitle}</p>}
 </div>
 </div>
 <span className={`absolute top-2 right-2 text-xs font-medium px-2 py-0.5 rounded-full ${b.active?'bg-green-500 text-white':'bg-gray-500 text-white'}`}>{b.active?'Active':'Inactive'}</span>
 </div>
