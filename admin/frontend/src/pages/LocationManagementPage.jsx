...
import {
  ...
  Loader2, Eye, ShieldCheck,
} from 'lucide-react';
import MapplsMap from '../components/MapplsMap';
import { pincodesApi, usersApi } from '../services/api';
...
export default function LocationManagementPage() {
 ...
 const fetchData = async () => {
    setLoading(true);
    try {
      const pRes = await pincodesApi.getAll();
 ...
 const handleTogglePincode = async (code, currentEnabled) => {
    try {
      await pincodesApi.update(code, { enabled: !currentEnabled });
      setPincodes((prev) =>
 ...
 <div>
 <h1 className="text-page-title text-gray-900">Location Management (India)</h1>
 ...
 {/* Tab Navigation */}
 <div className="flex items-center bg-gray-100 p-1 rounded-xl gap-1 text-xs font-medium">
 <button
 ...
 <div>
 <p className="text-xs font-medium text-muted uppercase">States Covered</p>
 <p className="text-page-title text-gray-900">{statesSet.size}</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-xs font-medium text-muted uppercase">Cities Covered</p>
 <p className="text-page-title text-gray-900">{citiesSet.size}</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-xs font-medium text-muted uppercase">Active Pincodes</p>
 <p className="text-page-title text-gray-900">
 ...
 <div>
 <p className="text-xs font-medium text-muted uppercase">Geocoding Engine</p>
 <p className="text-xs font-medium text-gray-900">Mappls REST API</p>
 </div>
 </div>
 </div>
 ...
 <h2 className="text-sm font-semibold text-gray-900">Serviceable Locations & Pincodes</h2>
 ...
 <span>Loading location dataù</span>
 </div>
 ) : (
 <div className="overflow-x-auto">
 <table className="w-full text-left text-xs border-collapse">
 <thead>
 <tr className="bg-gray-50 text-label text-xs font-medium border-b border-gray-100">
 ...
 <td className="p-3 font-semibold text-gray-900">{pin.code}</td>
 <td className="p-3 font-semibold text-gray-800">{pin.areaName || 'ù'}</td>
 <td className="p-3 font-semibold text-gray-800">{pin.city || 'ù'}</td>
 <td className="p-3 font-semibold text-gray-600">{pin.district || pin.city || 'ù'}</td>
 ...
 <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
 <h2 className="text-sm font-semibold text-gray-900">Customer Delivery Locations (India)</h2>
 <div className="overflow-x-auto">
 <table className="w-full text-left text-xs border-collapse">
 <thead>
 <tr className="bg-gray-50 text-label text-xs font-medium border-b border-gray-100">
 ...
 <td className="p-3 font-medium text-gray-900">{cust.name || cust.full_name || 'Customer'}</td>
 <td className="p-3 text-gray-600">
 <div>{cust.email}</div>
 <div className="text-xs text-muted">{cust.phone || cust.mobile_number}</div>
 </td>
 ...
 <td className="p-3 font-semibold text-gray-900">{cust.pincode || '530001'}</td>
 <td className="p-3 font-mono text-[10px] text-gray-500">
 ...
 className="px-3 py-1 rounded-lg text-[10px] font-medium bg-primary/10 text-primary hover:bg-primary/20 transition flex items-center gap-1 ml-auto"
 >
 ...
 <h2 className="text-sm font-semibold text-gray-900">Interactive Mappls Location Map</h2>
