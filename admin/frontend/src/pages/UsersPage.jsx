...
function UserDrawer({ user, orders, onClose, onToggleBlock }) {
 ...
 <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary font-semibold text-base flex-shrink-0">
 {(user.name||'?')[0].toUpperCase()}
 </div>
 <div className="flex-1 min-w-0">
 <p className="text-sm font-semibold text-gray-900 truncate">{user.name||'Unknown'}</p>
 <p className="text-xs text-muted font-mono">{user.id||user.uid||'...'}</p>
 <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${user.status==='blocked'?'bg-red-100 text-red-700':'bg-green-100 text-green-700'}`}>
 {user.status||'active'}
 </span>
 </div>
 <button onClick={()=>onToggleBlock(user)} className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-medium border transition flex-shrink-0 ${user.status==='blocked'?'bg-green-50 text-green-700 border-green-200 hover:bg-green-100':'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'}`}>
 ...
 <button key={t.id} onClick={()=>setTab(t.id)} className={`flex-1 py-3 text-xs font-medium border-b-2 transition ${tab===t.id?'border-primary text-primary':'border-transparent text-gray-400 hover:text-gray-600'}`}>{t.label}</button>
 ))}
 </div>
 ...
 <div><p className="text-xs font-medium text-muted">{label}</p><p className="text-xs font-semibold text-gray-800 mt-0.5 break-words">{val}</p></div>
 </div>
 ))
 ) : (
 ...
 <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-green-100 text-green-700">{o.status}</span>
 </div>
 <div className="flex justify-between"><p className="text-xs text-gray-600">{(o.items||[]).length} items</p><p className="text-sm font-semibold text-gray-900">{formatINR(o.totalAmount||0)}</p></div>
 </div>
 ))
 )}
 </div>
...
export default function UsersPage() {
 ...
 <div><h1 className="text-page-title text-gray-900">Users</h1>
 <p className="text-xs text-muted mt-0.5">{users.length} registered ... {users.filter(u=>u.status==='blocked').length} blocked</p></div>
 </div>
 ...
 <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center text-primary font-medium text-[10px] flex-shrink-0">{(u.name||'?')[0].toUpperCase()}</div>
 <p className="font-medium text-gray-800 truncate max-w-[100px]">{u.name||'Unknown'}</p>
 </div>
 </td>
 ...
 <td className="px-3 py-3"><span className="text-[10px] font-medium text-primary bg-primary/10 px-1.5 py-0.5 rounded-full">{u.pincode||'...'}</span></td>
 <td className="px-3 py-3 text-gray-400 text-[10px]">{fmtDate(u.registeredAt||u.createdAt)}</td>
 <td className="px-3 py-3"><span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border ${u.status==='blocked'?'bg-red-50 text-red-600 border-red-200':'bg-green-50 text-green-700 border-green-200'}`}><span className={`w-1.5 h-1.5 rounded-full ${u.status==='blocked'?'bg-red-500':'bg-green-500'}`}/>{u.status||'active'}</span></td>
 <td className="px-3 py-3">
