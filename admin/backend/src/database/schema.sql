-- ═══════════════════════════════════════════════════════════════════════════
-- UshaMart — Definitive Supabase PostgreSQL Schema  (v3)
-- ═══════════════════════════════════════════════════════════════════════════
-- HOW TO RUN:
--   Supabase Dashboard → SQL Editor → New Query → paste this file → Run
--
-- Safe to re-run: uses IF NOT EXISTS / ON CONFLICT DO NOTHING / CREATE POLICY
-- with conflict guards so you can apply it to an existing database without
-- destroying valid records.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── Enable uuid extension (handy for future use) ───────────────────────────
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ── Helper: drop and recreate all RLS policies for a table ─────────────────
-- (called at the bottom of this file)

-- ════════════════════════════════════════════════════════════════════════════
-- TABLE DEFINITIONS (CREATE IF NOT EXISTS — never drops existing data)
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.users (
  ...
  gender          TEXT,
  profile_pic     TEXT,
  -- wishlist is now a dedicated table; this column is kept for backward compat
  -- and will be ignored by the new wishlist controller
...
-- ── 8. wishlists ──────────────────────────────────────────────────────────────
-- One row per user × product. Replaces the old users.wishlist JSONB column.
...
-- ── 9. cart_items ─────────────────────────────────────────────────────────────
-- Server-side cart for logged-in users. Guests continue to use localStorage.
CREATE TABLE IF NOT EXISTS public.cart_items (
  id         TEXT PRIMARY KEY,
  user_id    TEXT NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
  -- cart_key is the composite key used by the frontend: "product_id::variant_unit"
  cart_key   TEXT NOT NULL,
  product_id TEXT NOT NULL REFERENCES public.products(id) ON DELETE CASCADE,
  variant    TEXT,            -- variant unit string, e.g. "500ml"
...
-- ── 17. sessions (kept for compatibility) ─────────────────────────────────────
...
-- RLS policies below protect direct Supabase client access (e.g., accidental
-- anon key exposure) and serve as a defence-in-depth layer.
--
-- Policy design:
--   • Public catalog tables (products, categories, banners, special_offers,
--     coupons, notifications, pincodes) — anyone can SELECT; only service
--     role can INSERT/UPDATE/DELETE (enforced by key, not policy).
--   • User-owned tables (user_addresses, orders, wishlists, cart_items) —
--     a customer can only see their own rows via auth.uid().
--   • users table — users can read/update their own row; admins see all.
...
BEGIN
  -- Drop any legacy "allow_all_*" policies that were too broad
...
-- "category-images" bucket kept for backward compat with old image URLs.
...
-- Storage: public read; authenticated or service role can write
