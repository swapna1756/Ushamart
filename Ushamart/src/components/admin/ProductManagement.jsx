...
import {
  ...
  Loader2, ImagePlus, Save, MoreVertical, RefreshCw
} from 'lucide-react';
import { getProductImage } from '../UserApp';
import { ConfirmDialog } from './ConfirmDialog';
import { storage } from '../../db/mockFirebase';
...
const UNIT_PRESETS = ['250g','500g','1kg','2kg','5kg','100ml','250ml','500ml','1L','2L','Pack','Piece','Dozen','__custom__'];
...
function ProductFormModal({ isOpen, onClose, onSave, editingProduct, categories, pincodes, addToast }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const STANDARD_UNITS = ['250g','500g','1kg','2kg','5kg','100ml','250ml','500ml','1L','2L','Pack','Piece','Dozen'];
    if (editingProduct) {
      const savedUnit   = editingProduct.unit || '';
      const isPreset    = STANDARD_UNITS.includes(savedUnit);
      const resolvedPreset = isPreset ? savedUnit : (savedUnit ? '__custom__' : '500g');
      setForm({
        ...EMPTY, ...editingProduct,
        variantList:       editingProduct.variantList       || [],
        pincodesAvailable: editingProduct.pincodesAvailable || [],
        unitPreset:        resolvedPreset,
        unit:              savedUnit,
  ...
  const set = (key, val) => setForm(prev => {
    ...
    if (key === 'unitPreset' && val !== '__custom__') next.unit = val;  // auto-fill unit from preset; custom = manual
    return next;
  });

  // image upload handler — tries Firebase Storage, falls back to base64
  const processImages = async (files) => {
    ...
    for (const f of Array.from(files)) {
      ...
      try {
        // Attempt Firebase/Supabase storage upload first (returns public URL or base64)
        const url = await storage.uploadImage(f);
        urls.push(url);
      } catch {
        // Final fallback: FileReader base64
  ...
  const handleSave = async () => {
    ...
    if (form.unitPreset === '__custom__' && !form.unit.trim()) {
      addToast('Please enter a custom unit/size.', 'error'); return;
    }
    // Ensure the saved unit is the actual text (never the sentinel '__custom__')
    const finalUnit = form.unitPreset === '__custom__'
      ? form.unit.trim()
      : form.unitPreset;
    setSaving(true);
    try {
      await onSave({ ...form, unit: finalUnit });
      onClose();
    } catch (err) {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-7 py-5 space-y-4">
          ...
          {/* Row 3: Category + Unit Preset (+ custom input when needed) */}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <select value={form.unitPreset} onChange={e => set('unitPreset', e.target.value)} className={fis}>
                  {['250g','500g','1kg','2kg','5kg','100ml','250ml','500ml','1L','2L','Pack','Piece','Dozen'].map(u => (
          ...
          {form.unitPreset === '__custom__' && (
            <div>
              <label className={lbl}>
                Custom Unit / Quantity <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input
                value={form.unit}
                onChange={e => setForm(prev => ({ ...prev, unit: e.target.value }))}
                placeholder="e.g. 200g, 750ml, Family Pack - 750g, 6 Pack, 1 Bottle…"
                className={fi}
                autoFocus
              />
              <p style={{ fontSize: '10px', color: '#9ca3af', marginTop: '4px' }}>
                Type any unit or quantity — no restrictions.
              </p>
            </div>
          )}
...
function QuickStockModal({ product, onClose, onSave, addToast }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await onSave(product.id, Number(stock));
      addToast(`Stock updated to ${stock} units`, 'success');
      onClose();
    } catch (e) {
      addToast('Failed to update stock: ' + e.message, 'error');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[9995] flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-pop"
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-black text-gray-900">Update Stock</h3>
          <button onClick={onClose} className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center">
            <X size={14} className="text-gray-500" />
          </button>
        </div>
        <p className="text-xs text-gray-500 mb-4 truncate font-medium">{product.name}</p>
        <div className="mb-5">
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider block mb-1.5">New Stock Quantity</label>
          <input
            type="number" min="0" value={stock}
            onChange={e => setStock(e.target.value)}
            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-xl font-black text-center text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            autoFocus
          />
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={handleSave} disabled={saving}
            className="flex-1 py-2.5 rounded-xl text-xs font-bold text-white transition flex items-center justify-center gap-1.5"
            style={{ background: saving ? '#93c5fd' : 'linear-gradient(135deg, #2563EB, #3B82F6)', boxShadow: saving ? 'none' : '0 4px 12px rgba(37,99,235,0.35)' }}>
...
export default function ProductManagement({ products, categories, pincodes, onSave, onDelete, onToggleStatus, onUpdateStock, addToast, onNavigateToCategories }) {
  ...
  const [stockModal, setStockModal]       = useState(null); // product for quick stock update
  const PER_PAGE = 10;
                  ...
                  <tr key={prod.id} className={`transition ${isSelected ? 'bg-primary/5' : 'hover:bg-gray-50/60'}`}>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button onClick={() => setStockModal(prod)} title="Update stock"
