    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 animate-fadeIn">
      <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-primary/5 to-white">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold text-gray-400">Powered by Mappls (MapmyIndia)</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="space-y-3">
            <button
              ...
              className="btn-press w-full py-3 px-4 rounded-xl border-2 border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-bold text-xs flex items-center justify-center gap-2 transition"
            >
            ...
            <form onSubmit={handleSearch} className="relative flex items-center">
              ...
              <button
                type="submit"
                disabled={searching}
                className="absolute right-2 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-bold hover:bg-primary-hover transition"
              >
            ...
            {searchResults.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden max-h-48 overflow-y-auto space-y-0.5">
                {searchResults.map((item, idx) => (
                  ...
                  >
                    <p className="text-xs font-bold text-gray-800">{item.placeName}</p>
                    <p className="text-[10px] text-gray-500 truncate">{item.fullAddress}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="bg-gray-50 rounded-2xl p-3 border border-gray-200 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px] font-medium pt-1 border-t border-gray-200/60">
              <div>
                <span className="text-gray-400 font-bold text-[9px] uppercase block">State & District</span>
                <span className="text-gray-800 font-semibold">{selectedLoc.state} / {selectedLoc.district}</span>
              </div>
              <div>
                <span className="text-gray-400 font-bold text-[9px] uppercase block">City & Area</span>
                <span className="text-gray-800 font-semibold">{selectedLoc.city} / {selectedLoc.area}</span>
              </div>
              <div>
                <span className="text-gray-400 font-bold text-[9px] uppercase block">Pincode</span>
                <span className="text-gray-900 font-black">{selectedLoc.pincode}</span>
              </div>
              <div>
                <span className="text-gray-400 font-bold text-[9px] uppercase block">Coordinates</span>
            ...
            <div
              className={`mt-2 p-2.5 rounded-xl border flex items-center gap-2 text-xs font-bold ${
                validating
            ...
            >
              {validating ? (
                <Loader2 size={14} className="spin flex-shrink-0" />
              ) : serviceable ? (
                <CheckCircle2 size={14} className="flex-shrink-0" />
              ) : (
                <AlertCircle size={14} className="flex-shrink-0" />
              )}
              <span>{validating ? 'Verifying pincode delivery status…' : validationMsg}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-xl border border-gray-200 text-xs font-bold text-gray-600 hover:bg-white transition"
          >
            Cancel
          </button>
          <button
            ...
            className="btn-press flex-1 py-3 rounded-xl text-xs font-bold text-white bg-primary hover:bg-primary-hover disabled:opacity-50 transition shadow-md"
          >
            Confirm Location
          </button>
        </div>
