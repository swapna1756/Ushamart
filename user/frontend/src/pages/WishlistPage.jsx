...
export default function WishlistPage() {
 ...
 {wished.length > 0 && (
 <p className="text-xs text-muted mt-0.5">{wished.length} saved item{wished.length !== 1 ? 's' : ''}</p>
 )}
 </div>
 {wished.length > 0 && (
 <button onClick={() => clearAll()}
 className="btn-press flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium text-red-500 border border-red-200 hover:bg-red-50 transition">
 ...
 <h2 className="text-base font-semibold text-gray-700">Your wishlist is empty</h2>
 <p className="text-sm text-gray-400 mt-1 mb-6">Tap the ? on any product to save it here</p>
 <button onClick={() => navigate('/home')}
 className="btn-press px-5 py-3 rounded-2xl text-sm font-medium text-white"
 ...
 className="cursor-pointer text-sm font-medium text-gray-900 leading-snug line-clamp-2 mt-0.5">
 {product.name}
 </p>
 {product.unit && <p className="text-xs text-muted mt-0.5">{product.unit}</p>}
 ...
 {disc > 0 && (
 <span className="text-[10px] font-medium text-green-600">{disc}% off</span>
 )}
 </div>

 {/* Stock status */}
 {!inStock && (
 <span className="inline-block mt-1 text-[10px] font-medium text-red-500 bg-red-50 px-2 py-0.5 rounded-full border border-red-200">
 Out of Stock
 </span>
 )}
 ...
 className="btn-press flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition flex-1 justify-center"
 style={{
 ...
 <p className="text-sm font-medium text-gray-500">Product no longer available</p>
