...
export default function HomePage() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center px-6">
        ...
        <h2 className="text-base font-black text-gray-700 mb-1">Could not load products</h2>
        <p className="text-xs text-gray-400 mb-6 max-w-xs">{error}</p>
        <button onClick={loadData}
          className="btn-press px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-primary hover:bg-primary-hover shadow-md">
          Try Again
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 pt-3 pb-2">
          ...
          <div className="flex items-center justify-between mb-2">
            <button onClick={() => { setPinInput(pincode); setPinError(''); setShowPinModal(true); }}
              ...
              <div>
                ...
                <div className="flex items-center gap-0.5">
                  <p className="text-xs font-bold text-gray-900">{pincode || 'Set pincode'}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-black text-gray-900">Usha<span className="text-primary">Mart</span></span>
            </div>
          </div>
          {/* Search bar */}
      ...
      <div className="max-w-2xl mx-auto px-4 pb-8 space-y-6 pt-4">
        {/* Carousel */}
          ...
          <div className="rounded-2xl overflow-hidden shadow-md relative cursor-pointer"
            ...
            <div className="flex items-center justify-between p-5 min-h-[140px]">
              <div className="flex-1 pr-3 z-10">
                {curOffer.badgeText && (
                  <span className="inline-block mb-2 px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full bg-black/20 text-white">
                    {curOffer.badgeText}
                  </span>
                )}
                <h2 className="text-xl font-black leading-tight text-gray-900 mb-2">{curOffer.title}</h2>
                {curOffer.subtitle && <p className="text-xs text-gray-600 mb-3">{curOffer.subtitle}</p>}
                <button className="px-5 py-2 rounded-full text-xs font-black text-white bg-primary shadow-md">
                  {curOffer.buttonText || 'SHOP NOW'}
                </button>
              </div>
              {curOffer.imageUrl && (
                <div className="w-28 h-28 rounded-xl overflow-hidden shadow-lg flex-shrink-0">
                  <img src={curOffer.imageUrl} alt={curOffer.title} className="w-full h-full object-cover"
        ...
        {categories.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/categories')} className="flex items-center gap-0.5 text-xs font-bold text-primary">
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
              {categories.slice(0, 10).map(cat => (
                <button key={cat.id} onClick={() => navigate(`/categories?cat=${cat.id}`)}
                  ...
                  <span className="text-[10px] font-bold text-gray-700 text-center w-16 leading-tight line-clamp-2">
        ...
        {featured.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-black text-gray-900">✨ Picked For You</h2>
              <button onClick={() => navigate('/categories')} className="flex items-center gap-0.5 text-xs font-bold text-primary">
        ...
        {todayDeals.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base font-black text-gray-900">🔥 Today's Deals</h2>
              <button onClick={() => navigate('/categories')} className="flex items-center gap-0.5 text-xs font-bold text-primary">
                View All <ChevronRight size={14} />
              </button>
            </div>
        ...
        {featured.length === 0 && todayDeals.length === 0 && visibleProducts.length > 0 && (
          <section>
            <h2 className="text-base font-black text-gray-900 mb-3">All Products</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        ...
        {visibleProducts.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm font-black text-gray-700">No Products Available</p>
            ...
            {pincode && (
              <button onClick={() => setPincode('')}
                className="btn-press mt-4 px-4 py-2 rounded-xl text-xs font-bold text-primary border border-primary hover:bg-primary/5 transition">
                Show All Pincodes
              </button>
            )}
            <button onClick={loadData}
              className="btn-press mt-3 px-4 py-2 rounded-xl text-xs font-bold text-white bg-primary hover:bg-primary-hover shadow-sm transition">
              Refresh
            </button>
          </div>
        )}
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}
          ...
          <div className="bg-white rounded-t-3xl w-full max-w-md p-6 animate-slideUp shadow-2xl"
            ...
            <h3 className="text-sm font-black text-gray-900 mb-1">Change Delivery Location</h3>
            ...
            <form onSubmit={handlePincodeSubmit} className="space-y-3">
              ...
              <button type="submit" disabled={pinInput.length !== 6}
                className="btn-press w-full py-3 rounded-xl text-sm font-bold text-white bg-primary hover:bg-primary-hover disabled:opacity-50 transition shadow-md">
                Apply Pincode
              </button>
            </form>
          </div>
